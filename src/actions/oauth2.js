import { oauth2, openPopup, pollPopup } from '../utils'
...
function exchangeFacebookCodeForToken({ oauthData, config, window, interval }) {
    return new Promise((resolve, reject) => {
        ...
        fetch(accessTokenUrl, { qs: params }, function (err, response, accessToken) {
            if (accessToken.error) {
                reject({ response });
            }
            // Step 2. Retrieve user's profile information.
            fetch(graphApiUrl, { qs: accessToken }, function (err, response, profile) {
                if (profile.error) {
                    reject({ response });
                }
